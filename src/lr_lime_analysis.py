"""
Logistic Regression + LIME baseline analysis for Iris dataset splits.

Mirrors the XGBoost workflow so model behavior and misclassification
...
DEFAULT_LIME_DIR = ROOT / "outputs" / "lr" / "lime_plots"
CLASS_NAMES = ["Iris-setosa", "Iris-versicolor", "Iris-virginica"]


def train_and_test_from_json(json_file: Path, drop_features: list[str] | None = None):
    """Train one LogisticRegression model per JSON dataset; test on the last row."""
    ...
    for dataset_name, ds in datasets.items():
        ...
        feature_importances.append(np.abs(model.coef_).mean(axis=0))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Logistic Regression + LIME Iris analysis."
    )
    parser.add_argument(
        "--json",
        type=Path,
